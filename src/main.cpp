...
static inline int generateMTRandom(unsigned int s, int range)
{
...
static inline int get9Counts(const uint256& hash)
{
	int count = 0;
        for (int i = 0; i < 4; i++)
...
long GetProofOfWorkBlockBonusRewardFactor(CBlockIndex* pindex) 
{
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // GrainCoin: check proof-of-stake block signature
    ...
    else
    {
        ...
        if(!fTestNet && fCheckSig)
        {
            ...
            // GrainCoin: check proof-of-work block signature
            if (isAfterCheckpoint && !CheckBlockSignature(false))
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Grain", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        assert(block.hashMerkleRoot == uint256("707dcbeaaf8634f0530e42db6028aecb463772d8b377b51948455ddee8e8dbf2"));
...
string GetWarnings(string strFor)
{
    ...
    if (!GetBoolArg("-nosynccheckpoints", false) && Checkpoints::IsSyncCheckpointTooOld(60 * 60 * 24 * 300) && !fTestNet && !IsInitialBlockDownload())
    {
        nPriority = 100;
